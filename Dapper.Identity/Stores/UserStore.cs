...
namespace Dapper.Identity.Stores
{
        ...
        IUserLockoutStore<ApplicationUser>, IUserRoleStore<ApplicationUser>, IUserAuthenticationTokenStore<ApplicationUser>, IUserStore<ApplicationUser>
    {
        ...
        public Task<IdentityResult> DeleteAsync(ApplicationUser user, CancellationToken cancellationToken = default(CancellationToken)) {
            ...
            user.Status = false;
            user.Trashed = true;
            user.RecordStatus = RecordStatus.DeleteMode;
        ...
        public Task<IdentityResult> UpdateAsync(ApplicationUser user, CancellationToken cancellationToken = default(CancellationToken)) {
            ...
            user.ModifiedDate = DateTime.UtcNow;
            user.RecordStatus = RecordStatus.EditMode;
            ...
            CancellationToken cancellationToken = default(CancellationToken))
        {
            ...
            user.Roles.Add(new ApplicationUserRole
            {
                ...
                RecordStatus = RecordStatus.NewMode,
                Status = true,
                Trashed = false,
        ...
        public async Task RemoveFromRoleAsync(ApplicationUser user, string roleName, CancellationToken cancellationToken = default(CancellationToken)) {
            ...
            if (role != null)
            {
                var userRole = (await _usersRolesTable.GetRolesAsync(user)).FirstOrDefault(x => x.RoleId == role.RoleId);
                if (userRole != null)
                {
                    userRole.ModifiedDate = DateTime.UtcNow;
                    userRole.Status = false;
                    userRole.Trashed = true;
                    userRole.RecordStatus = RecordStatus.DeleteMode;

                    await _usersRolesTable.UpdateAsync(userRole, user.Id);

                    //Context.Entry(userRole).Property(x => x.CreatedDate).IsModified = false;
                    //Context.Entry(userRole).Property(x => x.CreatedBy).IsModified = false;
        ...
        public async Task<bool> IsInRoleAsync(ApplicationUser user, string roleName, CancellationToken cancellationToken = default(CancellationToken)) {
            ...
            var role = await _rolesTable.FindByNameAsync(roleName);
            if (role != null && role.Status && !role.Trashed)
            {
                var userRole = (await _usersRolesTable.GetRolesAsync(user)).FirstOrDefault(x => x.RoleId == role.Id);
                return userRole != null && userRole.Status && !userRole.Trashed;
            }
            return false;
